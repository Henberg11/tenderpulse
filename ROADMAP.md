...
- [x] Free structured-field extraction for GeM's standardized labels (EMD required, bid end date, item category)
- [x] AI-generated executive summary, eligibility summary, risk factors (Gemini free tier)
- [ ] OCR fallback for scanned (non-text) PDFs
- [ ] Word document support
- [ ] Full-text + embedding search
- [ ] Verify EMD-amount extraction against a real EMD-required tender (only tested against an EMD-not-required example so far -- see comments in app/services/document_intelligence.py)

---

## Phase 3 — Alerts & Dashboard
- [x] Watchdog + email alerts (done earlier)
- [x] Mobile-friendly dashboard (dashboard/, hosted free on GitHub Pages, reads Supabase directly)
